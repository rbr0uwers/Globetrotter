...
function sortContent(prop) {
    let asc = evaluateCurrentSort(prop);
    if (prop === "title") {
        if (asc)
            locationArr.sort((a, b) => a[prop].localeCompare(b[prop]));
        else
            locationArr.sort((a, b) => b[prop].localeCompare(a[prop]));
    }
    else if (prop === "date") {
        if (asc)
            locationArr.sort((a, b) => a[prop].getTime() - b[prop].getTime());
        else
            locationArr.sort((a, b) => b[prop].getTime() - a[prop].getTime());
    }
    filterContent(currentPageFilter);
}
function evaluateCurrentSort(prop) {
